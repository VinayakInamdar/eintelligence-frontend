...
export class DeviceCategoryComponent implements OnInit {
  ...
  bsValue = new Date();
  date = new Date();
  startDate = new Date(new Date().setDate(new Date().getDate() - 31)).toISOString().split("T")[0]; audiencesubmenu: boolean;
      ...
      res => {
        this.campaignService.GetDeviceCategoryReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
          devicecategoryres => {
            ...
            var desktop = Object.assign({ name: 'Desktop' }, devicecategoryres['desktop'])
            var mobile = Object.assign({ name: 'Mobile' }, devicecategoryres['mobile'])
            var tablet = Object.assign({ name: 'Tablet' }, devicecategoryres['tablet'])
