...
@Component({
  selector: 'app-sources-mediums',
  templateUrl: './sources-mediums.component.html',
  styleUrls: ['./sources-mediums.component.scss']
})
export class SourcesMediumsComponent implements OnInit {
  ...
  getAnalyticsData(): void {
    ...
    this.campaignService.getGaAnalyticsReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
      res => {
        this.campaignService.GetTrafficSourcesMediumsReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
          sourcemediumsres => {
            this.reportsData = res;
            this.source = new LocalDataSource(sourcemediumsres['sourcemediumList']);
