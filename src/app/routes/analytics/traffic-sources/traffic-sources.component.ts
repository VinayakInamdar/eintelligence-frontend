...
export class TrafficSourcesComponent implements OnInit {
  ...
  bsValue = new Date();
  date = new Date();
  startDate = new Date(new Date().setDate(new Date().getDate() - 31)).toISOString().split("T")[0]; behaviorsubmenu: boolean;
  ;
  ...
  ngOnInit(): void {
    ...
    this.acqusitionsubmenu = true;
    this.showDiv(event, 'true', 'analytics', 'acqusitionsubmenu')
    // setTimeout(() => {
    //   this.rFactor()
    // }, 1000);
      ...
      res => {
        this.campaignService.GetTrafficSourcesReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
          trafficsourceres => {
            ...
            var direct = Object.assign({ defaultValue: 'Direct' }, trafficsourceres['totalData']['direct'])
            var organicSearch = Object.assign({ defaultValue: 'Organic Search' }, trafficsourceres['totalData']['organic'])
            var referral = Object.assign({ defaultValue: 'Referral' }, trafficsourceres['totalData']['referral'])
            var social = Object.assign({ defaultValue: 'Social' }, trafficsourceres['totalData']['social'])
            var display = Object.assign({ defaultValue: 'Display' }, trafficsourceres['totalData']['display'])
            var datatable = [direct, organicSearch, referral, social, display]
            this.trafficsourcetabledata = datatable;
  ...
  public showDiv(event, value, show, submenu) {
    ...
    this.show = show;
    this.acqusitionsubmenu = submenu == 'acqusitionsubmenu' ? true : false
