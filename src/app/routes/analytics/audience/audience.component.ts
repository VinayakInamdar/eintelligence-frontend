...
export class AudienceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.audiencesubmenu = true;
    this.showDiv(event, 'true', 'analytics', 'audiencesubmenu')
    // setTimeout(() => {
    //   this.rFactor()
    // }, 1000);
  ...
  getAnalyticsData(): void {
    ...
    this.campaignService.getGaAnalyticsReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
      res => {
        this.campaignService.GetAudienceReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
          audienceres => {
            this.reportsData = res;
            var newVisitors = Object.assign({ name: 'New Visitor' }, audienceres['newVisitors'])
            var returnVisitors = Object.assign({ name: 'Returning Visitor' }, audienceres['returnVisitors'])
            this.dateLabels = this.reportsData.gaPreparedDataDto.date;
            var datatable = [newVisitors, returnVisitors]
  ...
  public showDiv(event, value, show, submenu) {
    ...
    this.show = show;
    this.audiencesubmenu = submenu == 'audiencesubmenu' ? true : false;
