...
@Component({
  selector: 'app-site-speed',
  templateUrl: './site-speed.component.html',
  styleUrls: ['./site-speed.component.scss']
})
export class SiteSpeedComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.behaviorsubmenu = true;
    this.showDiv(event, 'true', 'analytics', 'behaviorsubmenu')
    // setTimeout(() => {
    //   this.rFactor()
    // }, 1000);
      ...
      res => {
        this.campaignService.GetGaBehaviorAnalyticsReports(this.selectedCampId, this.startDate, this.endDate).subscribe(
          behaviorres => {
            ...
            this.source = new LocalDataSource(behaviorres['siteSpeed']);
  ...
  public showDiv(event, value, show, submenu) {
    ...
    this.show = show;
    this.behaviorsubmenu = submenu == 'behaviorsubmenu' ? true : false;
